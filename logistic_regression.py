...
def categorical_to_numerical(data: pd.DataFrame):
    data['req_type'] = data['req_type'].map({'ambiente': 1, 'industria': 2, 'justica': 3})
    return data
...
print('Baseline: ' + str(round(max(raw_data['req_type'].value_counts()/len(raw_data))*100,2)) + '%')

result = cross_val_score(logistic_classifier, X, Y, scoring='accuracy', cv=10).mean()
